/*
 * Phase C — Domain 6 question bank builder
 ...
 * CISSP-D06-Q0001..0160, validates, and writes:
 *   questions/domain-06-security-assessment-testing.json
 *
...
if (questions.length !== 120) errors.push(`expected 120 questions, got ${questions.length}`);
...
const output = {
  meta: {
    ...
    generated_by: 'scripts/d6/build.cjs',
    ...
    id_range: `CISSP-D06-Q0001..CISSP-D06-Q${String(questions.length).padStart(4, '0')}`,
...
fs.writeFileSync(
  path.join(ROOT, 'questions/domain-06-security-assessment-testing.json'),
  JSON.stringify(output, null, 2) + '\n',
  'utf8'
);

console.log('Questions:', questions.length);
console.log('Difficulty:', JSON.stringify(diffTally), '(target e26/m60/h34)');
console.log('Types:', JSON.stringify(typeTally));
console.log('Errors:', errors.length);
errors.slice(0, 40).forEach(e => console.log('  !', e));
if (errors.length) process.exit(1);
console.log('OK — wrote questions/domain-06-security-assessment-testing.json');
