/*
 * Phase C — Domain 3 question bank builder
 ...
 * CISSP-D03-Q0001..0160, validates, and writes:
 *   questions/domain-03-security-architecture-engineering.json
 *
...
if (questions.length !== 130) errors.push(`expected 130 questions, got ${questions.length}`);
...
const output = {
  meta: {
    ...
    generated_by: 'scripts/d3/build.cjs',
    ...
    id_range: `CISSP-D03-Q0001..CISSP-D03-Q${String(questions.length).padStart(4, '0')}`,
...
fs.writeFileSync(
  path.join(ROOT, 'questions/domain-03-security-architecture-engineering.json'),
  JSON.stringify(output, null, 2) + '\n',
  'utf8'
);

console.log('Questions:', questions.length);
console.log('Difficulty:', JSON.stringify(diffTally), '(target e23/m61/h46)');
console.log('Types:', JSON.stringify(typeTally));
console.log('Errors:', errors.length);
errors.slice(0, 40).forEach(e => console.log('  !', e));
if (errors.length) process.exit(1);
console.log('OK — wrote questions/domain-03-security-architecture-engineering.json');
